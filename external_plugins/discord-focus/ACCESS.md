...
When `focusChannels` is set, DMs are filtered by their DM channel ID (not user ID). In practice, setting focus to guild channel IDs means DMs are dropped unless the DM channel ID is also included.
